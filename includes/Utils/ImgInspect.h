...
namespace ImageInspect
{
	inline void histogram(const TextureData& texData)
	{
		...
		const unsigned char* ptrCols= texData.getTextureData().data();
	...
	inline void inspect(const TextureData& texData, ImVec2 mouseUVCoord, ImVec2 displayedTextureSize)
	{
		...
		for(int32 y= -zoomSize; y <= zoomSize; y++)
		{
			...
			{
				ColourData  colData= texData.getTexelColor(coOrdX + x, coOrdY - y);
		...
		ColourData  colData= texData.getTexelColor(coOrdX + zoomSize / 2, coOrdX + zoomSize / 2);
		glm::u8vec3 pixData= colData.getColour_8_BitClamped();
		...
		ImVec4 color= ImColor(pixel.texel);
		ImVec4 colHSV;
		...
		ImGui::Separator();
		ImGui::Text("R 0x%02x  G 0x%02x  B 0x%02x", int32(color.x * 255.f), int32(color.y * 255.f),
								int32(color.z * 255.f));
		ImGui::Text("R %1.2f G %1.2f B %1.2f", color.x, color.y, color.z);
		ImGui::Separator();
		ImGui::Text("H 0x%02x  S 0x%02x  V 0x%02x", int32(colHSV.x * 255.f), int32(colHSV.y * 255.f),
								int32(colHSV.z * 255.f));
		ImGui::Text("H %1.2f S %1.2f V %1.2f", colHSV.x, colHSV.y, colHSV.z);
		ImGui::Separator();
		ImGui::Text("Alpha 0x%02x", int32(color.w * 255.f));
