...
namespace AstralRaytracer
{
	namespace Random
	{
		...
		{
			constexpr float32 oneOverFloatLimit=
					1.0f / static_cast<float32>(std::numeric_limits<uint32>::max());

			seed= pcgHash(seed);
			return static_cast<float32>(seed) * oneOverFloatLimit;
		}

		inline float32 randomFloatSymmetric(uint32& seed) { return randomFloat(seed) * 2.0f - 1.0f; }

		inline glm::vec3 unitSphere(uint32& seed)
		{
			return glm::normalize(glm::vec3(
					randomFloatSymmetric(seed), randomFloatSymmetric(seed), randomFloatSymmetric(seed)
			));
		}
