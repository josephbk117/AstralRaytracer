...
{
using UniformDataType = std::variant<int32, uint32, float32, glm::ivec2, glm::uvec2, glm::vec2, glm::ivec3, glm::uvec3,
                                     glm::vec3, glm::mat4>;
...
class ShaderProgram
{
  public:
    ...
    void compileShadersFromSrcCode(const std::string &vertexShaderSrcCode, const std::string &fragmentShaderSrcCode);
    void linkShaders();
    void addAttribute(const std::string &attributeName);
    void use() const;
    void unuse() const;

    [[nodiscard]] const std::unordered_map<std::string, UniformData> &getUniformData() const;
    [[nodiscard]] gl::GLint getUniformLocation(const std::string &uniformName) const;

    void setUniformValue(const std::string &uniformName, const UniformDataType &data, float32 min, float32 max);
  ...
  private:
    ...
    [[nodiscard]] uint32 createProgram(Errors::GenericError &error) const;
    [[nodiscard]] uint32 createVertexShader(Errors::GenericError &error) const;
    [[nodiscard]] uint32 createFragmentShader(Errors::GenericError &error) const;
