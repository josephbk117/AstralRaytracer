...
class StaticMesh : public Traceable
{
  public:
    ...
    StaticMesh(const std::vector<TriangleTraceable> &triangles, const AABB &boundingBox, const std::string &srcFilePath)
        : m_triangles(triangles), Traceable(boundingBox), m_sourceMeshFilePath(srcFilePath)
    {
    }

    [[nodiscard]] const std::string &getSourceMeshFilePath() const;
  ...
  private:
    [[nodiscard]] bool intersectsBoundingBox(const Ray &rayIn) const;
