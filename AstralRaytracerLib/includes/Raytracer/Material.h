...
class Material : public Serialization::Serializable
{
  public:
    ...
    Material(const ColourData &inAlbedo, const ColourData &inEmission, float32 inEmissionStr, float32 inRoughness)
        : albedo(inAlbedo), emission(inEmission), emissionStrength(inEmissionStr), roughness(inRoughness)
    {
    }
