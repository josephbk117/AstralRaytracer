...
void Compositor::processImage(const Scene &scene, const Resolution &resolution, TextureId inputTextureId)
{
    ...
    for (uint32 index = 1; index < scene.m_postProcessingStack.size(); ++index)
    {
        ...
        const uint32 selectedInputTex = (isOddIndex) ? m_renderTexture2.getTexture() : m_renderTexture1.getTexture();

        scene.m_postProcessingStack[index]->setMainColorTexture(inputTextureId);
        scene.m_postProcessingStack[index]->processImage(m_dwPanel, selectedRenderTex, selectedInputTex);
        m_outputTextureId = selectedRenderTex.getTexture();
    }
}

TextureId Compositor::getTextureId() const
{
