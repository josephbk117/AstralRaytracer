...
{
template <ArithmeticType T, uint32 ComponentCount> TextureData<T, ComponentCount>::TextureData(Resolution resolution)
{
    resize(resolution);
}
...
template <ArithmeticType T, uint32 ComponentCount> void TextureData<T, ComponentCount>::resize(Resolution resolution)
{
...
{
    ASTRAL_ASSERTM(m_resolution.x * m_resolution.y * ComponentCount == data.size(), "Mismatched data size");
    m_data = data;
}
...
template <ArithmeticType T, uint32 ComponentCount> int32 TextureData<T, ComponentCount>::getWidth() const noexcept
{
    return m_resolution.x;
}

template <ArithmeticType T, uint32 ComponentCount> int32 TextureData<T, ComponentCount>::getHeight() const noexcept
{
...
template <ArithmeticType T, uint32 ComponentCount>
void TextureData<T, ComponentCount>::setTexelColorAtPixelIndex(uint32 index,
...
template <ArithmeticType T, uint32 ComponentCount>
void TextureData<T, ComponentCount>::setTexelColor(uint32 x, uint32 y,
                                                   const glm::vec<ComponentCount, T, glm::defaultp> &color)
{
...
template <ArithmeticType T, uint32 ComponentCount>
glm::vec<ComponentCount, T, glm::defaultp> TextureData<T, ComponentCount>::getTexelColor(int32 x, int32 y) const
{
    const uint32 index = (m_resolution.x * y + x) * ComponentCount;
    ASTRAL_ASSERTM(index <= (m_data.size() - ComponentCount), "Invalid index");
...
template <ArithmeticType T, uint32 ComponentCount>
glm::vec<ComponentCount, T, glm::defaultp> TextureData<T, ComponentCount>::getTexelColor(
    float32 u, float32 v, SamplingType samplingType) const
{
