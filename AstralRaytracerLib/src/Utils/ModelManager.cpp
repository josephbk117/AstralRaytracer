...
AstralRaytracer::StaticMesh ModelManager::getStaticMeshFromGLTF(const std::filesystem::path &path)
{
    ...
    const tinygltf::Accessor &accessor = model.accessors[model.meshes.at(0).primitives.at(0).attributes["POSITION"]];
    ...
    const tinygltf::Accessor &indexAccessor = model.accessors[model.meshes.at(0).primitives.at(0).indices];
    const tinygltf::BufferView &indexBufferView = model.bufferViews[indexAccessor.bufferView];
    const tinygltf::Buffer &indexBuffer = model.buffers[indexBufferView.buffer];

    std::vector<TriangleTraceable> triangles;

    const int16 *meshIndices =
        reinterpret_cast<const int16 *>(&indexBuffer.data[indexBufferView.byteOffset + indexAccessor.byteOffset]);
    for (size_t i = 0; i < indexAccessor.count; i += 3)
    {
        triangles.push_back(TriangleTraceable(vertexPositions[meshIndices[i]], vertexPositions[meshIndices[i + 1]],
