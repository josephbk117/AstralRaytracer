...
{
Camera::Camera(float32 focalLength, float32 sensorHeight, float32 focusDistance, float32 apertureDiameter,
               float32 nearClip, float32 farClip)
    : m_focalLength(focalLength), m_sensorHeight(sensorHeight), m_focusDistance(focusDistance),
      m_apertureDiameter(apertureDiameter), m_nearClip(nearClip), m_farClip(farClip)
{
...
void Camera::moveForward(float32 units)
{
    m_position += m_forwardDir * units;
}
...
void Camera::moveUp(float32 units)
{
    m_position += MathConstants::UpDirection * units;
}
...
void Camera::rotate(glm::vec2 rot)
{
    ...
    glm::quat q = glm::normalize(
        glm::cross(glm::angleAxis(-pitchDelta, rightDir), glm::angleAxis(-yawDelta, MathConstants::UpDirection)));
...
void Camera::setFStop(float32 fStop)
{
    m_apertureDiameter = m_focalLength / fStop;
}

float32 Camera::getFStop() const
{
    return m_focalLength / m_apertureDiameter;
}

const Direction3D &Camera::getRight() const
{
    return m_right;
}

const Direction3D &Camera::getUp() const
{
    return m_up;
}
