...
{
	TriangleTraceable::TriangleTraceable(): m_vertexA(1.0f, 0.0f, 0.0f), m_vertexB(0.0f, 1.0f, 0.0f), m_vertexC(0.0f, 0.0f, 1.0f)
	{
		m_normal = glm::normalize(glm::cross(m_vertexC - m_vertexA, m_vertexB - m_vertexA));
	}

	TriangleTraceable::TriangleTraceable(const glm::vec3& vertexA, const glm::vec3& vertexB, const glm::vec3& vertexC): 
	m_vertexA(vertexA), m_vertexB(vertexB), m_vertexC(vertexC)
	{
		m_normal = glm::normalize(glm::cross(m_vertexC - m_vertexA, m_vertexB - m_vertexA));
	}

	float32 TriangleTraceable::getTriangleDistance() const
	{
		return glm::dot(m_normal, m_vertexA);
	}
