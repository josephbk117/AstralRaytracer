...
const Transform& DrawingPanel::getTransform() const noexcept { return transform; }

Transform& DrawingPanel::getTransform() noexcept { return transform; }
...
uint32 DrawingPanel::getTextureID() const noexcept { return this->textureID; }
...
glm::vec4 DrawingPanel::getPanelWorldDimension() noexcept
{
	...
	return glm::vec4(left + transform.getPosition().x, right + transform.getPosition().x,
									 top + transform.getPosition().y, bottom + transform.getPosition().y);
}
