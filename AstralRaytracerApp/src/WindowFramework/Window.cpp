...
void Window::imGuiInit()
{
    ...
    m_primaryFont = fontAtlas.AddFontFromFileTTF("app_assets/fonts/ABeeZee-Regular.ttf", primaryFontSize);
    m_secondaryFont = fontAtlas.AddFontFromFileTTF("app_assets/fonts/Roboto-Regular.ttf", secondaryFontSize);
    m_tertiaryFont = fontAtlas.AddFontFromFileTTF("app_assets/fonts/Roboto-Regular.ttf", tertiaryFontSize);
                                                  ...
                                                  const BaseRenderer &renderer, const Camera &cam, const Scene &scene)
{
    ...
    const Direction3D &rayDir =
        renderer.getRayDirectionFromNormalizedCoord(coOrd, cam.getInverseProjection(), cam.getInverseView());
    ...
    if (m_appState == ApplicationState::NONE)
    {
        ...
        if (Input::isKeyDown(*this, InputKey::T))
        {
            m_TransformOperation =
                isLeftShiftDown ? m_TransformOperation | IMGUIZMO_NAMESPACE::TRANSLATE : IMGUIZMO_NAMESPACE::TRANSLATE;
        }
        else if (Input::isKeyDown(*this, InputKey::S))
        {
            m_TransformOperation =
                isLeftShiftDown ? m_TransformOperation | IMGUIZMO_NAMESPACE::SCALE : IMGUIZMO_NAMESPACE::SCALE;
        }
        else if (Input::isKeyDown(*this, InputKey::R))
        {
            m_TransformOperation =
                isLeftShiftDown ? m_TransformOperation | IMGUIZMO_NAMESPACE::ROTATE : IMGUIZMO_NAMESPACE::ROTATE;
        }
    }
    ...
    if (ImGui::Begin("Main Window", nullptr, flags))
    {
        ...
        if (ImGui::BeginTable("viewportContentSplit", 1, tableFlags, ImGui::GetContentRegionAvail()))
        {
                                  ...
                                  {ImGui::GetContentRegionAvail().x, viewportSceneInfoSplitHeight}))
            {
                ...
                m_sceneHierarchy.display(appStateInfo, scene, assetManager, *this,
                ...
                const float32 scale = glm::min(availableRegion.x / (float32)appStateInfo.rendererResolution.x,
                ...
                ImVec2 gapRegion = {(availableRegion.x - newRegion.x) * 0.5f, (availableRegion.y - newRegion.y) * 0.5f};
                ...
                IMGUIZMO_NAMESPACE::SetRect(ImGui::GetWindowPos().x, ImGui::GetWindowPos().y, newRegion.x, newRegion.y);
                ...
                if (scene.hasSceneLoaded())
                {
                    ...
                    glm::mat4 transform =
                        scene.m_sceneTraceables.at(appStateInfo.selectedObjectIndex).get()->getTransformMatrix();

                    IMGUIZMO_NAMESPACE::Manipulate(glm::value_ptr(cam.getView()), glm::value_ptr(cam.getProjection()),
                                                   m_TransformOperation, IMGUIZMO_NAMESPACE::LOCAL,
                ...
                if (ImGui::BeginTabBar("MyTabBar", tab_bar_flags))
                {
                    ...
                    {
                        m_postProcessingStack.display(scene, *this, ImVec2(0.0f, viewportSceneInfoSplitHeight));
                        ImGui::EndTabItem();
                    }
                         ...
                         AstralRaytracer::UI::AppStateInfo &appStateInfo)
{
    ...
    if (ImGui::SliderInt("##Bounce Count", &bounceCount, 1, 32, "Bounce Count:%d", ImGuiSliderFlags_AlwaysClamp))
    {
    ...
    ImGui::SameLine();
    ImGui::SetNextItemWidth(sliderWidth);
    if (ImGui::SliderInt2("Resolution", reinterpret_cast<int32 *>(&appStateInfo.rendererResolution), 64, 1920, "%dpx",
...
void Window::drawMenuBar(Scene &scene, AssetManager &assetManager, UI::AppStateInfo &appStateInfo)
{
    if (ImGui::BeginMenuBar())
    {
        if (ImGui::BeginMenu("File"))
        {
            ...
            {
                const nfdu8filteritem_t filterItem[1] = {{"Project", "asproj"}};
            ...
            {
                const nfdu8filteritem_t filterItem[1] = {{"Scene", "ascene"}};
...
void Window::drawFrameratePlot()
{
    ...
    ImGui::PlotLines("##Framerate", frameTimeCopy.data(), frameTimeCopy.size(), 0, overlay.data(), 0.0f, maxValue);
}
